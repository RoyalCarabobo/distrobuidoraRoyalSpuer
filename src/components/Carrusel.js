...
export default function Carrusel() {
    ...
    const fetchLatestProducts = useCallback(async () => {
        try {
            const data = await ProductService.getLatestProducts(5);
            setProducts(data);
            
            // Si el índice actual excede la longitud después de actualizar, lo reiniciamos
    ...
    useEffect(() => {
        ...
        // Suscripción a Real-time para actualizar productos vivos
        const unsubscribe = ProductService.subscribeToLatestProducts(() => {
            console.log("Actualización en tiempo real detectada en Carrusel");
            fetchLatestProducts();
        });
    ...
    }, [currentIndex, products, handleNext]);
